...
public class MessageRepositoryJdbcImpl implements MessageRepository {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            if (resultSet.next()) {
                ...
                Chatroom chatroom = new Chatroom(resultSet.getLong("chatroom_id"), resultSet.getString("chatroom_name"),
                        null, null);
                if (resultSet.getTimestamp("message_datetime") != null) {
    ...
    public boolean update(Message message) throws SQLException {
        try (Connection connection = dataSource.getConnection()) {
            String sql = "UPDATE messages SET message_text = ?, message_datetime = ? WHERE message_id = ?";
            PreparedStatement statement = connection.prepareStatement(sql);
            statement.setString(1, message.getMessageText());

            if (message.getMessageDateTime() != null) {
                statement.setTimestamp(2, java.sql.Timestamp.valueOf(message.getMessageDateTime()));
            } else {
